...
try {
    ...
    $update_query = $conn->prepare("
        UPDATE turma
        SET nome_identificacao = 
        CASE 
            WHEN nome_identificacao = 'Professores' THEN CONCAT('Professores') 
    ...
    $stmt = $conn->prepare("
        SELECT *, CASE WHEN atividade = 1 THEN 'Ativo' ELSE 'Inativo' END AS situacao
        FROM turma
        ORDER BY 
            atividade DESC,   -- Ativos primeiro
            nome_identificacao ASC  -- Ordem alfabética
    ");
    if (!$stmt->execute()) {
