...
btnGenerate.addEventListener("click", () => {
  fetch("emprestimos.php")
    ...
    .then((data) => {
      ...
      // Adicionar estilo e cabeçalho do PDF uma única vez
      let cabecalho = `
      ...
      for (let chave in grupos) {
        ...
        // Adicionar cabeçalho e título da turma
        html += `
          <div style="page-break-before: always;">${cabecalho}</div>
          <h1 style="font-family: 'Poppins', sans-serif; text-align: center;">Turma ${serie} - ${curso}</h1>
        `;

        let meses = {};
        for (let emprestimo of grupos[chave]) {
          let mes = new Date(emprestimo.data_emprestimo).toLocaleString(
            "pt-BR",
            { month: "long" }
          );
          if (!meses[mes]) {
          ...
          html += `
            <table style="width: 90%; margin: 0 auto; border-collapse: collapse;">
              <tr>
                ...
                <th style="text-align: center; background-color: #f0f0f0; border: 1px solid #ddd; padding: 8px; font-weight: bold; font-family: 'Poppins', sans-serif;">Dias Atrasados</th>  <!-- Nova coluna adicionada -->
              </tr>
          `;
          for (let emprestimo of meses[mes]) {
            html += `
              <tr>
                ...
                <td style="text-align: center; border: 1px solid #ddd; padding: 8px; font-family: 'Poppins', sans-serif;">${emprestimo.dias_decorridos}</td>  <!-- Nova coluna adicionada -->
              </tr>
            `;
          }
          html += `</table>`;
